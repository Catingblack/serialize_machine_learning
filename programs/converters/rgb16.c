...
int rgb16_decode(char* filename, image_t* im, int little_endian)
{
	int i,pix;
	FILE* f_in;
	uint16_t buf[3];
...
int rgb16_encode(char* filename, image_t* im, int little_endian)
{
	int i=0,pix=0;
	FILE* f_out;
	int must_swap = host_little_endian() ^ little_endian;
...
int main()
{
	...
	for (little_endian = 0; little_endian < 2; little_endian++)
	{
		...
		rgb16_encode(filename, &im, little_endian);
		fprintf(stderr, "Decoding image\n", little_endian);
		rgb16_decode(filename, &im_out, little_endian);
