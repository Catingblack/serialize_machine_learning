...
int multi_buf_dump(multi_buf_t *mb, const char *filename, int dtype)
{
	return multi_buf_dump2(mb, filename, dtype, 0, 1, mb->n_buffers);
}

int multi_buf_dump2(multi_buf_t *mb, const char *filename, int dtype, int buf_offset, int buf_step, int n_bufs)
{
	int i;
	char filename_i[256];
	if (mb->dump_mode == DUMP_MODE_NONE)
		return 0;

	for (i = buf_offset; i < buf_offset + n_bufs; i+=buf_step)
	{
		if (mb->dump_mode == DUMP_MODE_BIN)
			sprintf(filename_i, DUMP_DIR "%s.%d.bin" , filename, i);
		else
			sprintf(filename_i, DUMP_DIR "%s.%d.txt" , filename, i);

		if (mb->fout[i] || (mb->fout[i] = fopen(filename_i, "wb")))
		{
			if (mb->dump_mode == DUMP_MODE_BIN)
			{
				fwrite( (void*) mb->bufs.s16[i], mb->element_size, mb->sizes[i], mb->fout[i]);
			}
			else
			{
				int j;
				for (j = 0; j < mb->sizes[i]; j++)
				{
					if (dtype == MB_DUMP_U16)
						fprintf(mb->fout[i], "%d,", mb->bufs.u16[i][j]);
					else if (dtype==MB_DUMP_S16)
						fprintf(mb->fout[i], "%d,", mb->bufs.s16[i][j]);
					else if (dtype==MB_DUMP_HEX_16)
						fprintf(mb->fout[i], "%04X,", mb->bufs.u16[i][j]);
					else if (dtype==MB_DUMP_HEX_32)
						fprintf(mb->fout[i], "%08X,", mb->bufs.u32[i][j]);
					else if (dtype==MB_DUMP_S32)
						fprintf(mb->fout[i], "%d,", mb->bufs.s32[i][j]);
					else if (dtype==MB_DUMP_U32)
						fprintf(mb->fout[i], "%d,", mb->bufs.u32[i][j]);
					if (j%10==9)
						fprintf(mb->fout[i], "\n");
				}
				fprintf(mb->fout[i], "\n\n");
		...
		{
			fprintf(stderr, "unable to open file %s. check folder exists.\n", filename_i);
		}
	}
	return 0;
}
