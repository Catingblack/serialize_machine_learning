...
static int gcli_budget_compute_generic(const precinct_t* prec,
										int gcli_method, 
										precinct_budget_table_t* pbt,
										predbuffer_t *residuals,
										int gc_nonsig_group_size,
										int run_mode,
										int sig_flags_group_width,
										int bounded_alphabet,
										uint64_t active_methods)
{
	...
	{
		sig_flags = sig_flags_malloc(precinct_max_gcli_width_of(prec), sig_flags_group_width);
	...
	{
		lvl = precinct_lvl_of(prec, position);
		...
		{				
			...
			if (method_uses_no_pred(gcli_method))
				...
				bgt_gcli += budget_bounded_code(res_buf_coded, predictors_buf, gtli, res_len_coded, bounded_alphabet, lvl);
			else
...
int dump_gcli_budget_table(const precinct_t *prec, uint32_t** budget_table, int mode)
{
	int mode_offset, gtli, lvl;
	uint32_t** budget_table_ptr;
	 	
	mode_offset = idx2d(mode,0,bands_count_of(prec));
	budget_table_ptr = budget_table + mode_offset;
	fprintf(stderr, "mode%d\n", mode);
	for (gtli = 0; gtli < 16; gtli++)
	{
		fprintf(stderr, "gtli=%2d ", gtli);
		for (lvl=0; lvl < bands_count_of(prec); lvl++)
		{
			fprintf(stderr, " %4d ,", budget_table_ptr[lvl][gtli]);
		}
		fprintf(stderr, "\n");
	}
	return 0;
...
void gcli_gtli2scenario_budget_table(
	uint32_t** budget_table,
	uint32_t** scenario_budget_table,
	int scenario2gtli_table[MAX_GCLI + 1][MAX_LVLS])
{
	...
	for (scenario = 0; scenario <= MAX_GCLI; scenario++)
	{
		for (lvl = 0; lvl < MAX_LVLS; lvl++) {
			scenario_budget_table[lvl][scenario] = 0;
		}

		for (lvl = 0; lvl < MAX_LVLS; lvl++) {
			if (scenario2gtli_table[scenario][lvl] >= 0)
			{
				assert(scenario2gtli_table[scenario][lvl] <= MAX_GCLI);
				scenario_budget_table[lvl][scenario] = budget_table[lvl][scenario2gtli_table[scenario][lvl]];
			}
		}
	}
}

static int compute_residuals(uint64_t active_methods,
				const precinct_t *prec,
				const precinct_t *prec_top,
				int* gtli_top_array,
				int gc_trunc,
				int gc_top_pred_mode,
				predbuffer_t *residuals)
{
	...
	for (pred = 0; pred < PRED_COUNT; pred++)
	{
		...
		for (gcli_method = 0; gcli_method < GCLI_METHODS_NB; gcli_method++)
		{
				...
				((method_get_run(gcli_method)==RUN_SIGFLAGS_ZRCSF) && (pred == PRED_NONE)))
			{
				for (lvl=0; lvl < bands_count_of(prec); lvl++)
				{
					for(ypos = 0; ypos < precinct_height_of(prec,lvl); ypos++)
					{
...
int fill_gcli_budget_table(uint64_t active_methods,
				const precinct_t *prec,
				const precinct_t *prec_top,
				int* gtli_top_array,
				precinct_budget_table_t* pbt,
				...
				int bounded_alphabet)
{
	...
	compute_residuals(active_methods, prec, prec_top,	gtli_top_array,
					gc_trunc, gc_top_pred_mode,	residuals);

	 
	for (method = 0; method < GCLI_METHODS_NB; method++)
	{
		...
		else
		{
			...
			{
				gcli_budget_compute_generic(prec,method, pbt, residuals, gc_nonsig_group_size,run_mode, sigflags_group_width, bounded_alphabet, active_methods);
			}
		}
	}
	return 0;
