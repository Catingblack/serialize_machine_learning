...
uint32_t budget_bounded_code(const gcli_pred_t* pred_buf, const gcli_data_t* predictor_buf, int gtli, int len, int alphabet, int band_idx);
...
int budget_get_data_budget(const gcli_data_t* gclis, int gclis_len, 
	uint32_t* budget_table, int table_size, int group_size, int include_sign);

int budget_get_sign_budget(const sig_mag_data_t* datas_buf,	int data_len,
							const gcli_data_t* gclis_buf, int gclis_len,
							uint32_t* budget_tables, int table_size, 
							int group_size, int dq_type);
